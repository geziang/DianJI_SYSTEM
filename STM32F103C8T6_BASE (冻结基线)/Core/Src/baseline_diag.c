...
#define BASELINE_DIAG_PERIODIC_LOG_ENABLE (0U)
...
static baseline_diag_result_t baseline_diag_check_control(void)
{
  ...
  baseline_diag_logf("[CTRL] state=%s %s",
                     (state == CONTROL_LOOP_STATE_SAFE_IDLE) ? "safe_idle" : "unknown",
                     baseline_diag_result_text(result));
  return result;
}
...
#if (BASELINE_DIAG_PERIODIC_LOG_ENABLE != 0U)
...
#else
  (void)baseline_diag_initialized;
#endif
}
